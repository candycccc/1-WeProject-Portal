...
// ── Font ──
const loadedFonts = new Set(['Figtree', 'Mulish', 'DM Sans']);
...
// ── Login photo ──
function setLoginCover(url) {
  const isVideo = /\.(mp4|webm|mov)$/i.test(url || '');
  document.querySelectorAll('.login-photo').forEach(photoEl => {
    const video = photoEl.querySelector('video.login-cover-video');
    if (isVideo) {
      ...
      if (video) { video.style.display = 'none'; video.pause && video.pause(); }
      photoEl.style.backgroundImage = url ? "url('" + url + "')" : 'linear-gradient(135deg,#2c3e50,#3498db)';
    }
  });
}
function pickPhoto(el) {
  ...
  el.classList.add('selected');
  setLoginCover(el.dataset.photo);
}
...
const DEFAULTS = {
  ...
  logoUrl: 'images/techonomy-logo.svg',
  loginPhoto: 'images/cover-video.mp4',
};
function resetBranding() {
  // reset mode
  currentMode = DEFAULTS.mode;
  document.body.classList.toggle('dark-mode', DEFAULTS.mode === 'dark');
  ...
  document.getElementById('bp-light-btn').classList.toggle('active', DEFAULTS.mode === 'light');
  document.getElementById('bp-dark-btn').classList.toggle('active', DEFAULTS.mode === 'dark');
  ...
  // login photo
  setLoginCover(DEFAULTS.loginPhoto);
  document.querySelectorAll('.bp-photo-opt').forEach(p => p.classList.remove('selected'));
...
(function restoreScreenOnLoad() {
  // Techonomy build: always start on the Login screen — never restore a
  // previously saved screen (the static HTML already marks #s-login "on").
  try { localStorage.removeItem('wq_screen_desktop'); } catch(e) {}
})();
...
// ── Techonomy client defaults: dark mode + purple accent + video cover + DM Sans ──
currentMode = DEFAULTS.mode;
document.body.classList.toggle('dark-mode', DEFAULTS.mode === 'dark');
applyAccent(DEFAULTS.accent);
setLoginCover(DEFAULTS.loginPhoto);
